...
class IpGateHttpTest(
    ...
    companion object {
        ...
        @JvmStatic
        fun startWireMock() {
            wm = WireMockServer(WireMockConfiguration.wireMockConfig().dynamicPort())
            wm.start()
        }
        ...
        fun props(reg: DynamicPropertyRegistry) {
            reg.add("ipcheck.base-url") { "http://localhost:${wm.port()}/json" }
        }
    }
    ...
    @Test
    fun `blocks by country (US) and logs 403`() {
        val ip = "8.8.8.8"
        wm.stubFor(
            get(urlEqualTo("/json/$ip"))
                .willReturn(
        ...
        val resp = rest.postForEntity(
            ...
            HttpEntity("whatever", headers), String::class.java
        )

        assertEquals(403, resp.statusCode.value())
        assertTrue(resp.body!!.contains("Blocked country: US"))
    ...
    @Test
    fun `blocks by ISP (AWS) even if country allowed`() {
        val ip = "3.3.3.3"
        wm.stubFor(
            get(urlEqualTo("/json/$ip"))
                .willReturn(
        ...
        assertEquals(403, resp.statusCode.value())
        assertTrue(resp.body!!.contains("Blocked ISP"))
    }

    fun waitForLog(predicate: (RequestLog) -> Boolean): RequestLog {
        val deadline = System.currentTimeMillis() + 2000
        while (System.currentTimeMillis() < deadline) {
            val match = repo.findAll().firstOrNull(predicate)
